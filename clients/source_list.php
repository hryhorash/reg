...
$sql = "SELECT * from sources WHERE archive=:archive ORDER BY name";
...
$handle['change'] = array(
	...
	'link_start'=>'/clients/source_edit.php?id=',
	'button'=>'<i class="fas fa-edit"></i>'
);
$handle['block'] = array(
	'title'=>lang::HANDLING_ARCHIVE, 
	'link_start'=>'/config/archive.php?id=',
	'link_finish'=>'&table=sources&archive=1&URL='.$_SERVER['PHP_SELF'].'&tab='.$_GET['tab'],
...
$handle['restore'] = array(
	...
	'link_start'=>'/config/archive.php?id=',
	'link_finish'=>'&table=sources&archive=0&URL='.$_SERVER['PHP_SELF'].'&tab='.$_GET['tab'],'button'=>'<i class="fas fa-trash-restore"></i>',
	'alertMSG'=>lang::HANDLING_RESTORE
);

$title=lang::MENU_CLIENT_SOURCES;
//---------------------VIEW--------------------------------------
...
echo '<section class="content">';
	...
	echo '<h2>'.lang::MENU_CLIENT_SOURCES.'</h2>';
	...
	if ($_GET['tab'] != 'archive') echo '<a class="button" href="/clients/source_add.php?tab=active">'.lang::BTN_ADD.'</a>';
	?>
</section>
