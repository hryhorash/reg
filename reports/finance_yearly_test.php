...
while(isset($years[$count])) {
	$stmt6 = $pdo->prepare("SELECT DISTINCT date 
										FROM visits
						WHERE locationID = :locationID
							...
							AND state = 10
						");
	$stmt6->bindValue(':year', $years[$count], PDO::PARAM_STR);
	$stmt6->bindValue(':locationID', $_SESSION['locationSelected'], PDO::PARAM_INT);
	$stmt6->execute();
	$visit_dates = $stmt6->fetchAll(PDO::FETCH_ASSOC);


	$stakes_sum[$count] = 0;
	$date_fin = $years[$count] . '-12-31';
	
	$i = 0;
	foreach($subcatIDs as $subcatID)
	{
		$total_subcategory = 0;
		$stmt7 = $pdo->prepare("SELECT date, unitPrice, monthlyPrice
								FROM stakes
								WHERE subcatID = :subcatID
									AND locationID = :locationID
									AND date BETWEEN '1970-01-01' AND :date
								ORDER BY date DESC");
		$stmt7->bindValue(':subcatID', $subcatID, PDO::PARAM_INT);
		$stmt7->bindValue(':locationID', $_SESSION['locationSelected'], PDO::PARAM_INT);
		$stmt7->bindParam(':date', $date_fin, PDO::PARAM_STR);
		$stmt7->execute();
		$stakes = $stmt7->fetchAll(PDO::FETCH_ASSOC);
		...
		switch(true){
			...
			case($unitPrice > 0):
				...
				foreach($visit_dates as $row_date) {
					foreach($stakes as $row) {
		...
		$stakes_sum[$count] += $total_subcategory;
		//$stakes_details[$count][] = '<p><span class="bold">' . $subcatNames[$i] . ':</span> ' . correctNumber($subcatID_sum) . '</p>';
		$stakes_details[$count][] = $subcatNames[$i] . ': ' . correctNumber($total_subcategory) . "\n";
		$i++;
