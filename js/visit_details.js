...
function wage_rate(el) {
  ...
  while (ratesJson[i]["userID"] > 0) {
    if (
      ratesJson[i]["userID"] == userID &&
      ratesJson[i]["specialtyID"] == catID
    )
...
$(document).on("change", function () {
  ...
  $("input[name='qty[]'], input[name='priceSold[]").on(
    "keyup blur",
    function () {
      ...
      if (q > 0 && p > 0) {
        ...
        $(this)
          ...
          .attr(
            "data-tooltip",
            profit_lable + ": " + totalNetto_this.toFixed(2)
          );
      }
      salesTotals();
    }
