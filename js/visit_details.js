...
//var ratesJson = get_staff_rates();
//setTimeout(function(){ alert(ratesJson); }, 3000);
//
//
//function get_staff_rates() {
//	var xhttp_rates = new XMLHttpRequest();
//		$.ajax({
//		type: "POST",
//		url: "get_staff_rates_ajax.php",
...
function delete_staff_total_list(staffID) {
	...
	if(doNothing != 1) {
		...
		$("input[name='price[]']").each(function(){
			var staffID_this = $(this).siblings("select[name='staff[]']").find('option:selected').val();
...
function deleteWork() {
	...
	if(isOld > 0) {
		$(this).siblings("input[name='workID[]']").val(0);
		$(this).siblings("select[name='staff[]']").append($('<option>').val(0).text('del')) //добавляем опцию с нулем
